...
function showSearchResults(b){console.log("showing results");searchResults=[];if(0==b.length)alert("No Results Found. :(");else{for(var a=0;a<b.length;a++){for(var c=b[a]._poi_loc.split("|"),c=new google.maps.LatLng(c[0],c[1]),f=b[a].title,f=f.split(" | "),d="<ul>",e=0;e<f.length;e++)d=d+"<li>"+f[e]+"</li>";d+="</ul>";c=new google.maps.Marker({position:c,map:map,animation:google.maps.Animation.DROP,icon:templateDir+"/images/poi-pink.png",html:d});google.maps.event.addListener(c,"click",function(){copyToClipboard(this)});
google.maps.event.addListener(c,"mouseover",function(){infowindow.setContent(this.html);infowindow.open(map,this)});searchResults.push(c)}b=new google.maps.LatLngBounds;for(a=0;a<searchResults.length;a++)searchResults[a].setMap(map),b.extend(searchResults[a].getPosition()),map.fitBounds(b)}}
...
b=binds;break;case "sbind":a=templateDir+"/images/poi-bind-safe.png";b=binds;break;case "craft":a=templateDir+"/images/poi-craft.png";b=crafts;break;case "portal":a=templateDir+"/images/poi-portal.png";b=portals;break;case "pchamber":a=templateDir+"/images/poi-chamber.png";b=portals;break;case "village":a=templateDir+"/images/poi-village.png";b=villages;break;case "chest":a=templateDir+"/images/poi-chest.png",b=chests}var f=poiArray[c]._poi_loc.split("|"),d=poiArray[c].title;null!=d&&d.match("|")&&
(d=d.split("|"));var e="<ul>";if(null!=d&&0<d.length)for(var g=0;g<d.length;g++)e=e+"<li>"+d[g]+"</li>";else e=e+"<li>"+d+"</li>";e+="</ul>";f=new google.maps.LatLng(f[0],f[1]);e=new google.maps.Marker({position:f,map:null,icon:a,html:e});google.maps.event.addListener(e,"click",function(){copyToClipboard(this)});google.maps.event.addListener(e,"mouseover",function(){infowindow.setContent(this.html);infowindow.open(map,this)});b.push(e)}}
