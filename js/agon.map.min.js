...
var marker,poiArray;function getUserLoc(a){window.console.log("Got Loc",a);a=a.split("|");window.console.log(a);a=new google.maps.LatLng(a[0],a[1]);placeMarker(a);map&&(map.setZoom(7),map.setCenter(a))}
function placeMarker(a){marker?(marker.setPosition(a),marker.title="Click Me!"):(marker=new google.maps.Marker({position:a,map:map,icon:templateDir+"/images/poi-default.png",title:"Click Me!"}),google.maps.event.addListener(marker,"click",function(){copyToClipboard(marker)}));window.location.hash=marker.getPosition().lat()+"|"+marker.getPosition().lng()}function copyToClipboard(a){window.prompt("Copy to clipboard: Ctrl+C, Enter",a.getPosition().lat()+"|"+a.getPosition().lng())}
function getPOIs(){mobs.length||(jQuery.ajax({url:templateDir+"/getPOIs.php",dataType:"json",success:function(a){poiArray=a.pois;createMarkers()},error:function(){alert("Problem with Map. Please contact us through the feedback form describing what happened! Thanks!")}}),""!=userPOI&&getUserLoc(userPOI))}
...
google.maps.event.addListener(e,"click",function(){copyToClipboard(e)});a.push(e)}}function showMarkers(a){var b;switch(a){case "mobs":b=mobs;break;case "banks":b=banks;break;case "crafts":b=crafts;break;case "binds":b=binds;break;case "portals":b=portals;break;case "holdings":b=holdings;break;case "villages":b=villages}for(var a=new google.maps.LatLngBounds,c=0;c<b.length;c++)b[c].setMap(map),a.extend(b[c].getPosition()),map.fitBounds(a)}
