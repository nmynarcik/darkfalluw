...
function showSearchResults(b){console.log("showing results");searchResults=[];if(0==b.length)alert("No Results Found. :(");else{for(var a=0;a<b.length;a++){for(var c=b[a]._poi_loc.split("|"),c=new google.maps.LatLng(c[0],c[1]),f=b[a].title,f=f.split(" | "),e="<ul>",d=0;d<f.length;d++)e=e+"<li>"+f[d]+"</li>";e+="</ul>";c=new google.maps.Marker({position:c,map:map,animation:google.maps.Animation.DROP,icon:templateDir+"/images/poi-pink.png",html:e});google.maps.event.addListener(c,"click",function(){copyToClipboard(this)});
...
(e=e.split("|"));var d="<ul>";if(null!=e&&0<e.length)for(var g=0;g<e.length;g++)d=d+"<li>"+e[g]+"</li>";else d=d+"<li>"+e+"</li>";d+="</ul>";f=new google.maps.LatLng(f[0],f[1]);d=new google.maps.Marker({position:f,map:null,icon:a,html:d});google.maps.event.addListener(d,"dblclick",function(){copyToClipboard(this)});google.maps.event.addListener(d,"click",function(a){map.setZoom(7);map.setCenter(a.latLng)});google.maps.event.addListener(d,"mouseover",function(){infowindow.setContent(this.html);infowindow.open(map,
this)});b.push(d)}}function showMarkers(b){var a;switch(b){case "mobs":a=mobs;break;case "banks":a=banks;break;case "crafts":a=crafts;break;case "binds":a=binds;break;case "portals":a=portals;break;case "holdings":a=holdings;break;case "villages":a=villages;break;case "chests":a=chests}for(var b=new google.maps.LatLngBounds,c=0;c<a.length;c++)a[c].setMap(map),b.extend(a[c].getPosition()),map.fitBounds(b)}
