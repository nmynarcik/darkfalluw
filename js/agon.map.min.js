function CustomMapType(){}CustomMapType.prototype.tileSize=new google.maps.Size(256,256);CustomMapType.prototype.maxZoom=7;CustomMapType.prototype.getTile=function(b,a,c){var c=c.createElement("DIV"),e=templateDir+"/images/tiles/",e=e+(a+"_"+b.x+"_"+b.y+".png");c.style.width=this.tileSize.width+"px";c.style.height=this.tileSize.height+"px";c.style.backgroundColor="#00000";c.style.backgroundImage="url("+e+")";return c};CustomMapType.prototype.name="Custom";CustomMapType.prototype.alt="Tile Coordinate Map Type";
...
function searchPOIs(b){for(var a=0;a<searchResults.length;a++)searchResults[a].setMap(null);searchArray=[];for(a=0;a<poiArray.length;a++)null!=poiArray[a].title&&null!=poiArray[a].title.toLowerCase().match(b.toLowerCase())&&searchArray.push(poiArray[a]);showSearchResults(searchArray)}
function showSearchResults(b){console.log("showing results");searchResults=[];if(0==b.length)alert("No Results Found. :(");else{for(var a=0;a<b.length;a++){for(var c=b[a]._poi_loc.split("|"),c=new google.maps.LatLng(c[0],c[1]),e=b[a].title,e=e.split(" | "),d="<ul>",f=0;f<e.length;f++)d=d+"<li>"+e[f]+"</li>";var d=d+"</ul>",g=new google.maps.Marker({position:c,map:map,animation:google.maps.Animation.DROP,icon:templateDir+"/images/poi-pink.png",html:d});google.maps.event.addListener(g,"click",function(){copyToClipboard(g)});
google.maps.event.addListener(g,"mouseover",function(){infowindow.setContent(this.html);infowindow.open(map,this)});searchResults.push(g)}b=new google.maps.LatLngBounds;for(a=0;a<searchResults.length;a++)searchResults[a].setMap(map),b.extend(searchResults[a].getPosition()),map.fitBounds(b)}}
...
b=binds;break;case "sbind":a=templateDir+"/images/poi-bind-safe.png";b=binds;break;case "craft":a=templateDir+"/images/poi-craft.png";b=crafts;break;case "portal":a=templateDir+"/images/poi-portal.png";b=portals;break;case "pchamber":a=templateDir+"/images/poi-chamber.png";b=portals;break;case "village":a=templateDir+"/images/poi-village.png";b=villages;break;case "chest":a=templateDir+"/images/poi-chest.png",b=chests}var e=poiArray[c]._poi_loc.split("|"),d=poiArray[c].title;null!=d&&d.match("|")&&
(d=d.split("|"));var f="<ul>";if(null!=d&&0<d.length)for(var g=0;g<d.length;g++)f=f+"<li>"+d[g]+"</li>";else f=f+"<li>"+d+"</li>";var f=f+"</ul>",e=new google.maps.LatLng(e[0],e[1]),h=new google.maps.Marker({position:e,map:null,icon:a,html:f});google.maps.event.addListener(h,"dblclick",function(){copyToClipboard(h)});google.maps.event.addListener(h,"click",function(a){map.setZoom(7);map.setCenter(a.latLng)});google.maps.event.addListener(h,"mouseover",function(){infowindow.setContent(this.html);infowindow.open(map,
this)});b.push(h)}}function showMarkers(b){var a;switch(b){case "mobs":a=mobs;break;case "banks":a=banks;break;case "crafts":a=crafts;break;case "binds":a=binds;break;case "portals":a=portals;break;case "holdings":a=holdings;break;case "villages":a=villages;break;case "chests":a=chests}for(var b=new google.maps.LatLngBounds,c=0;c<a.length;c++)a[c].setMap(map),b.extend(a[c].getPosition()),map.fitBounds(b)}
