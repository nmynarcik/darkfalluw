...
var marker,poiArray;function placeMarker(a){marker?(marker.setPosition(a),marker.title=a.lat()+"|"+a.lng()):(marker=new google.maps.Marker({position:a,map:map,icon:templateDir+"/images/poi-default.png",title:a.lat()+"|"+a.lng()}),google.maps.event.addListener(marker,"click",function(){copyToClipboard(marker)}))}
function copyToClipboard(a){marker==a?window.prompt("Copy to clipboard: Ctrl+C, Enter",marker.title):window.prompt("Copy to clipboard: Ctrl+C, Enter",a.getPosition().lat()+"|"+a.getPosition().lng())}function getPOIs(){mobs.length||jQuery.ajax({url:templateDir+"/getPOIs.php",dataType:"json",success:function(a){poiArray=a.pois;createMarkers()},error:function(){alert("Problem with Map. Please contact us through the feedback form describing what happened! Thanks!")}})}
