...
var map,CustomMapType=new CustomMapType,fullscreen=!1,showMobs=!1,showBanks=!1,showCrafts=!1,showBinds=!1,showPortals=!1,showChests=!1,showHoldings=!1,showVillages=!1,mobs=[],banks=[],binds=[],crafts=[],portals=[],chambers=[],chests=[],holdings=[],villages=[],searchArray=[],searchResults=[];
...
var marker,poiArray;function getUserLoc(b){b=b.split("%7C");b=new google.maps.LatLng(b[0],b[1]);placeMarker(b);map&&(map.setZoom(7),map.setCenter(b))}
...
function copyToClipboard(b){window.prompt("Copy to clipboard: Ctrl+C, Enter","http://darkfallunholywars.info/world-map/#"+b.getPosition().lat()+"|"+b.getPosition().lng())}function getPOIs(){mobs.length||(jQuery.ajax({url:templateDir+"/getPOIs.php",dataType:"json",success:function(b){poiArray=b.pois;createMarkers()},error:function(){alert("Problem with Map. Please contact us through the feedback form describing what happened! Thanks!")}}),""!=userPOI&&getUserLoc(userPOI))}
function searchPOIs(b){console.log("searching pois");for(var a=0;a<searchResults.length;a++)searchResults[a].setMap(null);searchArray=[];for(a=0;a<poiArray.length;a++)null!=poiArray[a].title&&null!=poiArray[a].title.toLowerCase().match(b.toLowerCase())&&searchArray.push(poiArray[a]);showSearchResults(searchArray)}
function showSearchResults(b){console.log("showing results");searchResults=[];for(var a=0;a<b.length;a++){var c=b[a]._poi_loc.split("|"),c=new google.maps.LatLng(c[0],c[1]),d=new google.maps.Marker({position:c,map:map,icon:templateDir+"/images/bullet.png",title:b[a].title});google.maps.event.addListener(d,"click",function(){copyToClipboard(d)});console.log("result",d);searchResults.push(d)}b=new google.maps.LatLngBounds;for(a=0;a<searchResults.length;a++)searchResults[a].setMap(map),b.extend(searchResults[a].getPosition()),
map.fitBounds(b)}
...
binds;break;case "sbind":a=templateDir+"/images/poi-bind-safe.png";b=binds;break;case "craft":a=templateDir+"/images/poi-craft.png";b=crafts;break;case "portal":a=templateDir+"/images/poi-portal.png";b=portals;break;case "pchamber":a=templateDir+"/images/poi-chamber.png";b=portals;break;case "village":a=templateDir+"/images/poi-village.png";b=villages;break;case "chest":a=templateDir+"/images/poi-chest.png",b=chests}var d=poiArray[c]._poi_loc.split("|"),d=new google.maps.LatLng(d[0],d[1]),e=new google.maps.Marker({position:d,
map:null,icon:a,title:poiArray[c].title});google.maps.event.addListener(e,"dblclick",function(){copyToClipboard(e)});google.maps.event.addListener(e,"click",function(a){map.setZoom(7);map.setCenter(a.latLng)});b.push(e)}}
function showMarkers(b){var a;switch(b){case "mobs":a=mobs;break;case "banks":a=banks;break;case "crafts":a=crafts;break;case "binds":a=binds;break;case "portals":a=portals;break;case "holdings":a=holdings;break;case "villages":a=villages;break;case "chests":a=chests}for(var b=new google.maps.LatLngBounds,c=0;c<a.length;c++)a[c].setMap(map),b.extend(a[c].getPosition()),map.fitBounds(b)}
