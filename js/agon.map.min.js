...
(e=e.split("|"));var d="<ul>";if(null!=e&&0<e.length)for(var g=0;g<e.length;g++)d=d+"<li>"+e[g]+"</li>";else d=d+"<li>"+e+"</li>";d=d+'</ul><a href="http://www.darkfallunholywars.info/wp-admin/post.php?post='+arr[g].pid+'&action=edit" class="icon icon-cog" target="_blank" title="Edit POI">Edit POI</a>';f=new google.maps.LatLng(f[0],f[1]);d=new google.maps.Marker({position:f,map:null,icon:a,html:d});google.maps.event.addListener(d,"click",function(){copyToClipboard(this)});google.maps.event.addListener(d,
