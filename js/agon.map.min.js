...
function showSearchResults(b){console.log("showing results");searchResults=[];if(0==b.length)alert("No Results Found. :(");else{for(var a=0;a<b.length;a++){for(var c=b[a]._poi_loc.split("|"),c=new google.maps.LatLng(c[0],c[1]),e=b[a].title,e=e.split(" | "),d="<ul>",f=0;f<e.length;f++)d=d+"<li>"+e[f]+"</li>";var d=d+"</ul>",g=new google.maps.Marker({position:c,map:map,icon:templateDir+"/images/bullet.png",html:d});google.maps.event.addListener(g,"click",function(){copyToClipboard(g)});google.maps.event.addListener(g,
"mouseover",function(){infowindow.setContent(this.html);infowindow.open(map,this)});searchResults.push(g)}b=new google.maps.LatLngBounds;for(a=0;a<searchResults.length;a++)searchResults[a].setMap(map),b.extend(searchResults[a].getPosition()),map.fitBounds(b)}}
