...
function showSearchResults(b){console.log("showing results");searchResults=[];if(0==b.length)alert("No Results Found. :(");else{for(var a=0;a<b.length;a++){for(var c=b[a]._poi_loc.split("|"),c=new google.maps.LatLng(c[0],c[1]),e=b[a].title,e=e.split(" | "),d="<ul>",f=0;f<e.length;f++)d=d+"<li>"+e[f]+"</li>";var d=d+"</ul>",g=new google.maps.Marker({position:c,map:map,icon:templateDir+"/images/bullet.png",title:b[a].title,html:d});google.maps.event.addListener(g,"click",function(){copyToClipboard(g)});
...
(d=d.split("|"));var f="<ul>";if(null!=d&&0<d.length)for(var g=0;g<d.length;g++)f=f+"<li>"+d[g]+"</li>";else f=f+"<li>"+d+"</li>";var f=f+"</ul>",e=new google.maps.LatLng(e[0],e[1]),h=new google.maps.Marker({position:e,map:null,icon:a,title:poiArray[c].title,html:f});google.maps.event.addListener(h,"dblclick",function(){copyToClipboard(h)});google.maps.event.addListener(h,"click",function(a){map.setZoom(7);map.setCenter(a.latLng)});google.maps.event.addListener(h,"mouseover",function(){infowindow.setContent(this.html);
infowindow.open(map,this)});b.push(h)}}function showMarkers(b){var a;switch(b){case "mobs":a=mobs;break;case "banks":a=banks;break;case "crafts":a=crafts;break;case "binds":a=binds;break;case "portals":a=portals;break;case "holdings":a=holdings;break;case "villages":a=villages;break;case "chests":a=chests}for(var b=new google.maps.LatLngBounds,c=0;c<a.length;c++)a[c].setMap(map),b.extend(a[c].getPosition()),map.fitBounds(b)}
