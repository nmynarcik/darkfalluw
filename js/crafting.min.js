...
c+"</ul>").popover()},calculate:function(a){var c=a.Name,d=b("#item-count").val(),a="<strong>"+a.Quantity*crafter.itemCount+'</strong> <span class="theName"> '+c+"</span>: ",e;for(e in crafter.item.Recipe)a=a+d*crafter.item.Recipe[e]+" "+e,Object.keys(crafter.item.Recipe)[Object.keys(crafter.item.Recipe).length-1]!=e&&(a+=", ");return a},changeStyle:function(){for(var a=b(".theName:first").text().split(" "),c=0;c<a.length;c++)if("militant"==a[c].toLowerCase()||"stoic"==a[c].toLowerCase()||"barbaric"==
a[c].toLowerCase())a[c]=crafter.style;a=a.join(" ");b(".theName").text(a);a=b("#item-details").find("#thumb img").attr("src");a=a.replace("militant",crafter.style).replace("barbaric",crafter.style).replace("stoic",crafter.style);b("#item-details").find("#thumb img").attr("src",a);b("#item-details .btn-group .btn").removeClass("active");b('#item-details .btn[data-style="'+crafter.style+'"]').addClass("active")}};b("#trade_select").change(function(){crafter.getData(b(this).val())});b("#select-two").change(function(){if(crafter.advanced)crafter.filterList(b(this).find("option:selected").val());
