...
QuantityFormatter;e.width=75;break;case "emptybottle":e.formatter=CheckmarkFormatterStr;e.width=75;break;case "additional":e.formatter=AdditionalFormatter;e.width=300;break;case "minlevel":case "maxlevel":case "gold":e.width=50}f.push(e)}a=jQuery.csv.toObjects(a);craftingDataView=new Slick.Data.DataView;craftingDataView.beginUpdate();craftingDataView.setItems(a);craftingDataView.setFilter(function(a){for(var i in c)if(void 0!==i&&""!==c[i]){var e=d.getColumns()[d.getColumnIndex(i)],b=c[i].toLowerCase(),
g=!1;">"==b.charAt(0)&&(g=!0,b=b.substring(1));var h=!1;"<"==b.charAt(0)&&(h=!0,b=b.substring(1));var f=!1;isFinite(a[e.field])&&(f=!0);if(!f&&-1==a[e.field].toLowerCase().indexOf(c[i].toLowerCase())||f&&g&&parseFloat(b)>=parseFloat(a[e.field].toLowerCase())||f&&h&&parseFloat(b)<=parseFloat(a[e.field].toLowerCase())||f&&!g&&!h&&a[e.field].toLowerCase()!=b)return!1}return!0});craftingDataView.endUpdate();var d=new Slick.Grid("#craftingGrid",craftingDataView,f,{enableCellNavigation:!0,enableColumnReorder:!0,
multiColumnSort:!1,headerRowHeight:30,explicitInitialization:!0,showHeaderRow:!0,rowHeight:64});craftingDataView.onRowCountChanged.subscribe(function(){d.updateRowCount();d.render()});craftingDataView.onRowsChanged.subscribe(function(a,c){d.invalidateRows(c.rows);d.render()});var j=null;d.onSort.subscribe(function(a,c){var b=c.sortCol.field,e=c.sortAsc?1:-1,g=j;j=function(c,a){var d=c[b],f=a[b];return d==f&&g?g(c,a):!isFinite(d)||!isFinite(f)?(d==f?0:d>f?1:-1)*e:(parseFloat(d)==parseFloat(f)?0:parseFloat(d)>
parseFloat(f)?1:-1)*e};craftingDataView.sort(j);d.invalidate();d.render()});jQuery(d.getHeaderRow()).delegate(":input","change keyup",function(){var a=jQuery(this).data("columnId");null!=a&&(c[a]=jQuery.trim(jQuery(this).val()),craftingDataView.refresh())});d.onHeaderRowCellRendered.subscribe(function(a,b){"icon"!=b.column.id&&"copypasta"!=b.column.id&&(jQuery(b.node).empty(),jQuery("<input class='filter' type='text'>").data("columnId",b.column.id).val(c[b.column.id]).appendTo(b.node))});d.init();
d.setSelectionModel(new Slick.CellSelectionModel);d.registerPlugin(new Slick.AutoTooltips);d.getCanvasNode().focus();d.registerPlugin(new Slick.CellExternalCopyManager);jQuery("input.filter").attr("placeholder","Filter")})};
