...
var d='<h3><span class="theName">'+a.Name+"</span></h3><p>",d=d+"<strong>Skill:</strong> "+a.Skill+"<br>",d=d+"<strong>Min Level:</strong> "+a["Min Level"]+"<br>",d=d+"<strong>Max Level:</strong> "+a["Max Level"]+"<br>",d=d+'<ul class="ingredients">',e;for(e in a.Recipe)d=d+"<li><strong>"+e+":</strong> "+a.Recipe[e]+"</li>";c.find(".ingredients .details").html(d+"</ul></p>");c.find(".recipe .well").html(crafter.calculate(a));c.find("#item-count").val(crafter.itemCount);b("#item-details").removeClass().addClass(a.Skill.toLowerCase()).html(c);
crafter.changeStyle()},calculate:function(a){var c=a.Name,a=b("#item-count").val(),c="<strong>"+crafter.itemCount+'</strong> <span class="theName"> '+c+"</span>: ",d;for(d in crafter.item.Recipe)c=c+a*crafter.item.Recipe[d]+" "+d,Object.keys(crafter.item.Recipe)[Object.keys(crafter.item.Recipe).length-1]!=d&&(c+=", ");return c},changeStyle:function(){for(var a=b(".theName:first").text().split(" "),c=0;c<a.length;c++)if("militant"==a[c].toLowerCase()||"stoic"==a[c].toLowerCase()||"barbaric"==a[c].toLowerCase())a[c]=
crafter.style;a=a.join(" ");b(".theName").text(a);a=b("#item-details").find("#thumb img").attr("src");a=a.replace("militant",crafter.style).replace("barbaric",crafter.style).replace("stoic",crafter.style);b("#item-details").find("#thumb img").attr("src",a);b("#item-details .btn-group .btn").removeClass("active");b('#item-details .btn[data-style="'+crafter.style+'"]').addClass("active")}};b("#trade_select").change(function(){crafter.getData(b(this).val())});b("#select-two").change(function(){if(crafter.advanced)crafter.filterList(b(this).find("option:selected").val());
else for(var a=RegExp("\\b"+b("#select-two").val(),"gi"),c=0;c<crafter.allItems.length;c++)crafter.allItems[c].Name.match(a)&&crafter.showItem(crafter.allItems[c])});b("#select-three").change(function(){var a=RegExp(b("#select-three").val()+"\\s(.*)\\s?\\b"+b("#select-two").val(),"gi");b("#select-three").val()===b("#select-two").val()&&(a=RegExp("^"+b("#select-three").val()+"$","i"));for(var c=0;c<crafter.filteredList.length;c++)crafter.filteredList[c].Name.match(a)&&crafter.showItem(crafter.filteredList[c])});
b("#mastery").change(function(){crafter.mastery="checked"==b(this).attr("checked")?!0:!1;""!=b("#trade_select").val()&&crafter.getData(b("#trade_select").val())});b("#item-count").live("change",function(){crafter.itemCount=b(this).val();b("#item-details .recipe .well").html(crafter.calculate(crafter.item));crafter.changeStyle()});b(".btn.styleSwitch").live("click",function(){crafter.style=b(this).data("style");crafter.changeStyle()});b(".recipe .well").live("click",function(){window.prompt("Copy Recipe",
b(this).text())})})(jQuery);Array.prototype.inArray=function(b){for(var a=0;a<this.length;a++)if(b(this[a]))return!0;return!1};Array.prototype.pushIfNotExist=function(b,a){this.inArray(a)||this.push(b)};
