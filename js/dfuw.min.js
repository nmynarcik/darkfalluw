...
'<div id="server-status"><h2>Server Status:</h2>',c;for(c in d)d.hasOwnProperty(c)&&(b+='<span class="'+c+'">'+c.toUpperCase()+'<i class="icon '+d[c]+'"></i></span>');b+="</div>";a("#server-status").remove();a("#branding").append(b);a("#server-status").fadeIn()},error:function(d,b){var c;c='<div id="server-status"><h2>Server Status:</h2>'+("Error: "+b+"</div>");a("#server-status").remove();a("#branding").append(c);a("#server-status").fadeIn()}});setTimeout(function(){g.getStatus()},12E4)}},k={init:function(){a(".your-subject").change(function(){"Add My Clan"==
